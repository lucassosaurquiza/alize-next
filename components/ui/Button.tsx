...
const buttonVariants = cva(
  'active:scale-95 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus:outline-none',
  {
    variants: {
      variant: {
        default:
          'bg-gradient-to-r from-red-500 to-red-300 hover:bg-slate-200',
  ...
  ({ className, children, variant, isLoading, size, ...props }, ref) => {
    return (
      <button
        ...
        disabled={isLoading}
        {...props}>
        {isLoading ? <Loader2 className='mr-2 h-4 w-4 animate-spin text-white' /> : null}
        {children}
      </button>
    )
  }
